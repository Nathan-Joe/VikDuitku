...
JLoader::import('duitku', VIKDUITKU_DIR);
...
{
	// make sure the driver is duitku
	if (!$payment->isDriver('duitku'))
	{
		return;
	}
...
/**
	...
	$transient = set_transient('vikduitku_vikbooking_' . $payment->get('oid') . '_' . $payment->get('sid'), $payment->get('total_to_pay'), 10 * MINUTE_IN_SECONDS);
...
// restore cache flag
	...
	if(!$transient){
		...
		$fp = fopen(VIKDUITKU_DIR . DIRECTORY_SEPARATOR . 'VikDuitku' . $txname , 'w+');
...
add_action('payment_before_validate_transaction_vikbooking', function($payment)
{
	...
	$path = VIKDUITKU_DIR . DIRECTORY_SEPARATOR . 'Duitku' . DIRECTORY_SEPARATOR . $txname;
	/**
	 * Force the system to avoid using the cache for transient.
	 * The previous value will be reset after terminating the callback.
	 *
	...
	$transient = 'vikduitku_vikbooking_' . $payment->get('oid') . '_' . $payment->get('sid');
 ...
 * by using the duitku gateway.
 *
 * @since 1.0
 */
class VikBookingDuitkuPayment extends AbstractDuitkuPayment
{
	/**
	 * @override
	 * Class constructor.
